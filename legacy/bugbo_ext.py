# bugbo_ext.py | misc functions for inat observations
# Copyright (C) 2019-2024  EraserBird, person_v1.32, hmmm
...
from bugbo_functions import decrypt_chacha

logger = logging.getLogger("bugbo")


class Bugbo(commands.Cog):
    ...
    @app_commands.describe(
        ...
        name="The insect name that corresponds to the asset.",
    )
    async def observation(self, ctx: commands.Context, code: str, *, name: str):
        ...
        if not guess:
            await ctx.send(
                "Please provide the insect name to get the original asset.",
                ephemeral=True,
            )
            return
        ...
        if correct or ((await self.bot.is_owner(ctx.author)) and guess == "please"):
            ...
            await ctx.send(
                "**Sorry, that's not the correct insect.**\n*Please try again.*",
                ephemeral=True,
            )


async def setup(bot):
    await bot.add_cog(Bugbo(bot))
